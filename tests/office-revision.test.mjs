...
test('September 10: approval, dispatch, delivery failure, acceptance and work remain distinct', () => {
  ...
  assert.ok(state.tasks.every(t => t.status === 'pending_delivery' && t.attempts === 0))
  ...
  assert.deepEqual(state.tasks.map(t => t.status), ['delivery_failed', 'delivered'])
  assert.throws(() => act(state, 'send_tasks'), { code: 'NO_PENDING_TASKS' })
  state = act(state, 'accept_task', 'sales', { taskId: 'T-SALES' })
  assert.equal(state.tasks[1].status, 'accepted')
  assert.ok(state.tasks[1].acceptedAt)
  assert.equal(state.tasks[1].startedAt, undefined)
