...
test('HTTP full chain, confirmation, retries, role/version guards, isolation and restart durability', async () => {
  ...
  try {
    ...
    const staleAction = await a('/preview', { action: { type: 'request_quality' }, expectedVersion: cancelled.revision })
    assert.equal(staleAction.status, 409, 'analysis from an old revision cannot prepare a fresh action')
    assert.equal(staleAction.body.error.code, 'STALE_VERSION')
    assert.equal((await a('/snapshot')).body.state.tasks.length, 0)
    assert.equal((await a('/preview', { action: { type: 'request_quality' }, expectedVersion: 'latest' })).status, 422)
    const currentAction = { action: { type: 'request_quality' }, expectedVersion: late.state.revision }
    assert.equal((await a('/preview', currentAction)).body.preview.allowed, true)
    assert.equal((await a('/preview', currentAction, 'sales')).body.preview.allowed, false, 'matching revision does not bypass role checks')
