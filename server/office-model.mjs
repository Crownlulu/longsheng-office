...
const ACTIONS = ['send_tasks', 'accept_task', 'select_plan', 'approve_keep_a', 'request_quality', 'start_task', 'submit_quality', 'approve_switch', 'submit_receipt', 'close_matter', 'retry_delivery', 'set_arrival', 'arm_delivery_failure', 'reset']
      ...
      action: {
        type: 'object', properties: {
          type: {
            ...
            description: 'select_plan：采购或负责人选择 A/B，必须提供 supplierId；evidence 为可选补充理由，选择不等于批准；approve_keep_a：负责人确认已选 A 的风险并创建待发送的采购销售跟进任务，默认沿用选择理由；request_quality：采购或负责人创建/重新发起供应商 B 质量核验（尚无 T-QA 时使用，无需 taskId）；send_tasks：业务负责人批准后另行确认发送部门任务；accept_task：收件人确认接收已送达任务；start_task：任务接收人开始一个已经存在且已送达的任务，必须提供真实 taskId，不能用于创建核验；submit_quality：质量角色提交核验结果及证据；approve_switch：负责人在核验通过后批准切换；submit_receipt：采购/销售提交自己任务的回执；close_matter：负责人复核回执后关闭事项；retry_delivery：重试已存在的发送失败任务；set_arrival/arm_delivery_failure/reset：负责人调整演示数据、设置下次发送失败或重置。',
          },
...
export async function runOfficeChat({ state, question, role, provider, variant = 'ontology', mode = 'live', signal }) {
  ...
  const run = {
    ...
    facts: { state: structuredClone(state), analysis: analyze(state) },
  }
  ...
  try {
    ...
    if (mode === 'rules') {
      ...
      if (questionIs(/方案|切换|改用|不满意/)) {
        run.answer = `${question.startsWith('方案调整反馈：') ? '已收到你的调整约束。当前为规则演示，只能重新梳理已知 A/B 方案，不能验证新增业务条件或生成新供应商方案。\n' : ''}${result.options.map(option => `${option.name}：预计 D${option.arrivalDay} 到料，${option.riskCount} 条到料风险；${option.supplierId === 'B' ? '需要方案质量核验及负责人批准' : '沿用现有安排，但须确认并跟进到料风险'}。`).join('\n')}\n当前有效决定 ${result.effectiveDecisionId}。选择、批准、发送分开确认。[DOC-LEDGER] [DOC-RULES]`
