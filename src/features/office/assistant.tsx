  ...
  async function submit(input = question) {
    ...
    try {
      ...
      updateSession({
        run: result.run,
        question: '',
        messageIds: [...ids, result.run.id],
        failedQuestion: result.run.status === 'failed' ? requested : '',
      })
      await refresh()
    ...
    <div className='office-chat'>
      <aside className='office-chat-history' aria-label='历史会话'>
        ...
        {ownHistory.map((item) => (
          <button
            ...
            disabled={busy}
            className={
              'mb-2 block w-full rounded-lg border p-3 text-left hover:bg-muted ' +
              (run?.id === item.id ? 'border-primary bg-muted' : '')
            }
            onClick={() =>
              updateSession({
                run: item,
                ...
                historical: true,
                error: '',
                failedQuestion: item.status === 'failed' ? item.question : '',
              })
            }
          >
      ...
      <div className='office-chat-main'>
        <div className='flex flex-wrap items-center justify-between gap-3 border-b bg-card p-4'>
          <div>
            <h2 className='font-semibold'>{state.matter.title}</h2>
            <p className='mt-1 text-sm text-muted-foreground'>
              {state.matter.id} · {roleNames[role]}
            </p>
          </div>
          <Button variant='outline' size='sm' onClick={viewMatter}>
            查看事项详情
          </Button>
        </div>
        <div className='office-chat-messages' aria-live='polite'>
          {!messages.length && (
            <div className='py-8'>
              <MessageSquare className='mb-4 size-7 text-primary' />
              <h3 className='text-xl font-semibold'>从一个问题开始办理</h3>
              <p className='mt-3 text-sm leading-7 text-muted-foreground'>
                供应商 A 原定 D{state.suppliers[0]?.originalDay}，现预计 D
                {state.suppliers[0]?.arrivalDay}{' '}
                到料。你可以先查询影响或核对会议依据。
              </p>
            </div>
          )}
          ...
          {messages.map((item) => (
            <div key={item.id} className='mb-6 space-y-3'>
              <div className='ml-auto max-w-[90%] rounded-xl bg-primary/10 p-4 text-sm leading-7 break-words whitespace-pre-wrap'>
                {item.question}
              </div>
              <Section
                title={historical ? '历史回答' : '查询结果'}
                aside={<Badge variant='outline'>数据 v{item.revision}</Badge>}
              >
                {item.revision !== state.revision && (
                  <p className='mb-3 text-sm text-muted-foreground'>
                    保留查询当时的回答与数据；当前事项已更新为 v{state.revision}
                    。
                  </p>
                )}
                ...
                {item.status === 'completed' &&
                  item.facts &&
                  /方案|切换|改用|不满意/.test(item.question) && (
                    <div
                      className='mt-4 grid gap-3 sm:grid-cols-2'
                      aria-label='查询时方案快照'
                    >
                      {item.facts.analysis.options.map((option) => (
                        <div
                        ...
                        >
                          ...
                          <p className='mt-2'>
                            预计 D{option.arrivalDay} · {option.riskCount}{' '}
                            条到料风险
                          </p>
                ...
                {item.status === 'completed' &&
                  item.facts &&
                  /影响|订单|到料/.test(item.question) &&
                  !/方案|切换|改用|不满意/.test(item.question) && (
                    <div className='mt-4'>
                      <Orders
                        snapshot={{
                          ...snapshot,
                          state: item.facts.state,
                          analysis: item.facts.analysis,
                        }}
                      />
                    </div>
                  )}
          ...
          {busy && (
            <p role='status' className='flex items-center gap-2 p-4 text-sm'>
          ...
          {historical && (
            <Button
              variant='outline'
              onClick={() => updateSession({ historical: false })}
            >
              返回当前事项办理
            </Button>
          )}
          {!historical &&
            ...
            run.status === 'completed' && (
              <Button
                variant='outline'
                onClick={() => updateSession({ showPlans: true })}
              >
                继续比较并选择方案
              </Button>
            )}
            ...
            <Section
              ...
              aside={
                <Badge variant='outline'>
                  {plan
                    ? `已选 ${plan} · ${analysis.executionApproved ? '已批准' : '未批准'}`
                    : '待选择'}
                </Badge>
              }
            >
                ...
                {analysis.options.map((option) => (
                  ...
                  >
                    ...
                    <p className='mt-3 text-sm leading-6 text-muted-foreground'>
                      {option.supplierId === 'A'
                        ? '沿用现有供货安排；到料风险须由负责人确认并跟进。'
                        : '可提早到料；必须完成方案质量核验和负责人批准。'}
                    </p>
                    <Button
                      className='mt-4 w-full'
                      variant={
                        plan === option.supplierId ? 'default' : 'outline'
                      }
                      disabled={
                        !canSelect ||
                        actionBusy ||
              ...
              {canSelect && (
                <div className='mt-4 space-y-3'>
                  ...
                  <Textarea
                    ...
                    placeholder='系统会带出方案依据；你也可以补充考虑因素。'
                  />
                  <Button
                    variant='outline'
              ...
              <p className='mt-3 text-sm text-muted-foreground'>
                选择仅记录意向，尚需批准及单独确认发送任务。
              </p>
            </Section>
          )}
          {showWork && (
            <div className='mt-5 space-y-5'>
              <div role='region' aria-label='当前办理'>
                <RoleAction
                  title={
              ...
              {analysis.executionApproved && (
                <Section title='采购、销售任务状态'>
                  <Tasks
                    snapshot={{
                      ...snapshot,
                      state: {
                        ...state,
                        tasks: state.tasks.filter((t) => t.id !== 'T-QA'),
                      },
                    }}
                    role={role}
                    busy={busy || actionBusy}
              ...
              {(analysis.canClose || state.matter.status === 'closed') && (
                <ReviewSummary
                  snapshot={snapshot}
                  sources={sources}
        ...
        >
          ...
          <Textarea
            ...
            maxLength={4000}
            disabled={busy}
            onChange={(e) => updateSession({ question: e.target.value })}
            placeholder='输入问题，或描述你想进一步了解的内容…'
      ...
      <Dialog open={feedbackOpen} onOpenChange={setFeedbackOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>希望方案如何调整？</DialogTitle>
            <DialogDescription>
              保留原方案和选择记录。新建议需要重新选择、核验和批准；不会自动执行。
            </DialogDescription>
          </DialogHeader>
          <Textarea
            aria-label='方案修改反馈'
            ...
            placeholder='例如：优先保障最早需要物料的订单，请说明还缺哪些依据。'
          />
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                ...
                void submit(
                  `方案调整反馈：${feedback.trim()}。请依据现有 A/B 事实重新比较方案并回应这些约束；不能满足时说明缺口，不编造新供应商、价格或日期。原选择仅供参考，不执行任何动作。`
                )
                setFeedback('')
              }}
            >
