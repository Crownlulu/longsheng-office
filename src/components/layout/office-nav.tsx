...
export function OfficeNav() {
  ...
  return <SidebarGroup><SidebarGroupLabel>工作空间</SidebarGroupLabel><SidebarMenu>{items.map(item => <SidebarMenuItem key={item.id}><SidebarMenuButton asChild tooltip={item.label} isActive={page.split('?')[0] === item.id || (item.id === 'matter' && page.startsWith('matter-detail'))}><a href={`${import.meta.env.BASE_URL}office#${item.id}`}><item.icon /><span>{item.label}</span></a></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarGroup>
}
